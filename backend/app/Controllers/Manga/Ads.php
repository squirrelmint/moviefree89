...
class Ads extends Controller {
    ...
    public function index($branch_id, $search = "") {
        ...
        if (!empty($_GET)) {
            $page = $_GET['page'];
        } $nav = ['module_name' => 'โฆษณา', 'module_level' => array(array('name' => "Home", 'url' => "/"), array('name' => "Branch", 'url' => "/management/branch/" . $branch_id), array('name' => "Ads", 'url' => "/mangaads/branch/" . $branch_id . "/index"))];
    ...
    public function add($branch_id) {
        if (checkLogin() == false) {
            ...
            'module_level' => array(
                ...
                array('name' => "Ads", 'url' => "/mangaads/branch/" . $branch_id . "/index"), 
                array('name' => "Add Ads", 'url' => "/mangaads/branch/" . $branch_id . "/add"),
                )
            ];
    ...
    public function edit($branch_id, $id) {
        if (checkLogin() == false) {
            ...
            'module_level' => array(
                ...
                array('name' => "Branch", 'url' => "/management/branch/" . $branch_id),
                array('name' => "Ads", 'url' => "/mangaads/branch/" . $branch_id . "/index"), 
                array('name' => "Edit Ads", 'url' => "/mangaads/branch/" . $branch_id . "/edit/id/" . $id),)];
        echo view('template/header');
        echo view('template/body', $nav);
        $tem = $_SESSION['brtemp'][$branch_id];
        $data = ['branch_id' => $branch_id, 'ads_id' => $id , 'ads' => $this->adsdataModel->get_ads_onid($id),'posads' => get_temads( $tem )];
    ...
    public function saveadd($branch_id) {
        ...
        $data = ['branch_id' => $branch_id, 'ads_name' => $params['ads_name'], 'ads_url' => $params['ads_url'], 'ads_position' => $params['ads_position']];
        $this->validation->setRules(['ads_name' => 'required', 'ads_url' => 'valid_url', 'ads_position' => 'required']);
    ...
    public function saveedit($branch_id, $id) {
        ...
        $data = ['ads_name' => $params['ads_name'], 'ads_url' => $params['ads_url'], 'ads_position' => $params['ads_position']];
        $this->validation->setRules(['ads_name' => 'required', 'ads_url' => 'valid_url', 'ads_position' => 'required']);
        if ($this->validation->run($data) == FALSE) {
            setAlert('error', 'Add failed.');
            return redirect()->to('/mangaads/branch/' . $branch_id . '/index');
