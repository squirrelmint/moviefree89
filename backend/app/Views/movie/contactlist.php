...
<div class="panel panel-flat table-responsive">
    ...
    <table class="table datatable-basic">
        ...
        <thead>
            <tr>
                <th>#</th>
                <th style="width: 70%;" class="text-center">ข้อมูลติดต่อ</th>
                <th class="text-center">Actions</th>
            </tr>
        </thead>
        <tbody>
        <?php $i=$contact['start_row']; 
            if( !empty($contact['list']) ){
                foreach($contact['list'] as $ca){
        ?>
                <tr>
                    <td><?=$i?></td>
                    <td class="text-center"><?=$ca['mo_contact_data']?></td>
                    <td class="text-center">
                        <ul class="icons-list">
                            <li><a onclick="goDel('<?=$branch ?>','<?=$ca['mo_contact_id']?>')"><i class="text-danger-600 icon-trash"></i></a></li>
                        </ul>
                    </td>
                </tr>
                <?php 
    ...
    <div class="text-right pb-10 pt-10">
        <?= pagination($contact['page'], $contact['total_page']); ?>
    </div>
</div>
<!-- /basic datatable -->
<script>
    ...
    function goSearch(){
        window.location.href = "/movie/contact/branch/"+<?= $branch ?>+"/search/"+$("#txt_search").val();
    }

    function goClear(){
        window.location.href = "/movie/contact/branch/"+<?= $branch ?>;
    }
    ...
    bootbox.confirm({
        ...
        callback: function (result) {
            if (result==true) {
                window.location.href = "/movie/contact/branch/"+branch+"/del_contact/"+id;
            }
        }
    });
