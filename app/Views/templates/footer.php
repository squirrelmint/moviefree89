  ...
  <script>
    ...
    function goViewAv(id, name, type) {
      countView(id);
      var url = '';
      if (type == 'se') {
        url = "<?= base_url('av') ?>/series/" + id + '/' + decodeURI(name);
      } else {
        url = "<?= base_url('av') ?>/video/" + id + '/' + decodeURI(name);
      }
      window.open(url, '_blank');
    }

    function goViewCl(id, name, type) {
      countView(id);
      var url = '';
      if (type == 'se') {
        url = "<?= base_url('clip') ?>/series/" + id + '/' + decodeURI(name);
      } else {
        url = "<?= base_url('clip') ?>/video/" + id + '/' + decodeURI(name);
      }
      window.open(url, '_blank');
    }
    ...
    // SEARCH AV
    // $(document).ready(function() {
    //   $('#movie-formsearchav').submit(function(e) {
    //     goSearchav();
    ...
    // function goSearchav() {
    //   var searchav = $.trim($("#movie-searchav").val())
    //   if (searchav) {
    //     window.location.href = "/search/" + $("#movie-searchav").val();
    //   } else {
